...
// Data file format: ID \t X,Y (one row per sample)
// Truth file format: cluster format (one line per cluster, space-separated samples)
fn generate_blobs(
    data_file: &str,
    truth_file: &str,
) -> Result<(), Box<dyn std::error::Error>> {
    let mut data_out = String::new();
    let mut truth_clusters: Vec<Vec<String>> = Vec::new();
    ...
    truth_clusters.push(cluster3);

    let mut truth_out = String::new();
    for cluster in &truth_clusters {
        writeln!(truth_out, "{}", cluster.join("\t"))?;
    }
...
fn test_clust_pipeline_full() {
    ...
    let (_stdout, stderr) = NecomCmd::new()
        .args(&[
            "cut", "simple", &tree_file, "--k", "3", "--format",
    ...
    // Compare cut result (pair format) with ground truth (cluster format:
    // cluster_id\tsample_id, generated by `generate_blobs`).
    // Output ARI should be 1.0 for perfect clustering
    let (stdout, stderr) = NecomCmd::new()
        .args(&[
            "eval",
            "partition",
            &cut_file, // Prediction (pair format)
            "--other",
            &truth_file, // Ground Truth (cluster format)
            "--input-format",
            "pair",
            "--other-format",
            "cluster",
    ...
    // Skip the header line (which starts with "ari") and parse the first data row.
    let ari_line = stdout
        .lines()
        .find(|l| !l.trim().is_empty() && !l.trim().starts_with("ari"))
