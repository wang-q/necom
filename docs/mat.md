...
`necom mat` 模块专注于**距离矩阵（Distance Matrix）**的操作与转换。它是 `necom clust`（聚类与构树）的上游数据准备与预处理工具集。
...
### 1. PHYLIP 距离矩阵 (Dense)
`necom` 内部使用 `NamedMatrix` 结构存储，底层为 `CondensedMatrix`（一维数组存储上三角或下三角），内存占用约为 $O(N^2/2)$。
...
- **Relaxed**: 允许长名称，通常以 Tab 分隔。`necom` 默认支持读取此格式。
