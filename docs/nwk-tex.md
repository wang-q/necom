...
`necom nwk to-forest` and `necom nwk to-tex` convert Newick trees into LaTeX code using the [Forest](https://ctan.org/pkg/forest) package. This is useful for producing publication-quality phylogenetic trees directly from the command line.
...
[Tectonic](https://tectonic-typesetting.github.io/) is the recommended compiler, but `latexmk -xelatex` also works.
